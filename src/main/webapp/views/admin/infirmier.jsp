        ...
        <header class="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg border border-white/50 p-6 mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">Gestion des Infirmiers</h1>
                        <p class="text-sm text-gray-600 mt-1">Liste et gestion du personnel infirmier</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <aside class="lg:col-span-1">
                <nav class="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg border border-white/50 p-6 sticky top-6">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <p class="text-xs font-medium text-gray-500 mb-3">Statistiques Infirmiers</p>
                        <div class="space-y-2">
                            <%
                                List<Medcen> allInfirmiers = (List<Medcen>) request.getAttribute("infirmiers");
                                int totalInfirmiers = allInfirmiers != null ? allInfirmiers.size() : 0;
                            %>
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-gray-600">Total infirmiers</span>
                                <span class="font-semibold text-purple-600"><%= totalInfirmiers %></span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-gray-600">Dernier ajout</span>
                                <span class="font-semibold text-blue-600">
                                    <%
                                        if (totalInfirmiers > 0 && allInfirmiers.get(0).getId() != null) {
                                            out.print(allInfirmiers.get(0).getId());
                                        } else {
                                            out.print("--");
                                        }
                                    %>
            ...
            <main class="lg:col-span-3">
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">Liste des Infirmiers</h2>
                        <p class="text-sm text-gray-600 mt-1">Gérez le personnel infirmier de votre établissement</p>
                    </div>
                    <a href="<%= request.getContextPath() %>/admin/create"
                ...
                <div class="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg border border-white/50 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-blue-50">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-bold text-gray-900">Personnel Infirmier</h3>
                                <p class="text-sm text-gray-600 mt-1"><%= totalInfirmiers %> infirmier(s) trouvé(s)</p>
                            </div>
                            <div class="flex items-center gap-3">
                                <div class="relative">
                                    <input type="text" placeholder="Rechercher un infirmier..."
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Infirmier</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Contact</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Mot de passe</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Rôle</th>
                            </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-100">
                            <%
                                List<Medcen> infirmiers = (List<Medcen>) request.getAttribute("infirmiers");
                                if (infirmiers != null && !infirmiers.isEmpty()) {
                                    for (Medcen infirmier : infirmiers) {
                            %>
                            <tr class="hover:bg-purple-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-600">
                                    <%= infirmier.getId() != null ? infirmier.getId() : "N/A" %>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-purple-400 to-blue-400 flex items-center justify-center text-white text-sm font-semibold shadow-md">
                                            <%= (infirmier.getNomComplet() != null && !infirmier.getNomComplet().isEmpty()) ? infirmier.getNomComplet().substring(0,1).toUpperCase() : "I" %>
                                        </div>
                                        <div class="text-left">
                                            <p class="text-sm font-medium text-gray-900"><%= infirmier.getNomComplet() %></p>
                                            <p class="text-xs text-gray-500">ID: <%= infirmier.getId() != null ? infirmier.getId() : "N/A" %></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900"><%= infirmier.getEmail() %></div>
                                    <div class="text-sm text-gray-500"><%= infirmier.getTele() != null ? infirmier.getTele() : "Non renseigné" %></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <code class="text-xs bg-gray-100 px-2 py-1 rounded font-mono text-gray-700">
                                            <%= infirmier.getPassword() != null ? infirmier.getPassword().substring(0, Math.min(8, infirmier.getPassword().length())) + "..." : "N/A" %>
                                        </code>
                                        <button class="text-blue-600 hover:text-blue-800 text-xs"
                                                onclick="copyToClipboard('<%= infirmier.getPassword() != null ? infirmier.getPassword() : "" %>')"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-purple-100 text-purple-800">
                                        <%= infirmier.getRole() != null ? infirmier.getRole().name() : "INFIRMIER" %>
                                    </span>
                                </td>
                            </tr>
                            <%
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-4 text-lg font-medium text-gray-900">Aucun infirmier trouvé</p>
                                    <p class="mt-2 text-sm text-gray-500">Commencez par ajouter votre premier infirmier à l'équipe.</p>
                                    <a href="<%= request.getContextPath() %>/admin/create"
                    ...
                    <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                        <div class="flex items-center justify-between">
                            <div class="text-sm text-gray-700">
                                Affichage de <span class="font-medium">1</span> à <span class="font-medium"><%= totalInfirmiers %></span> sur <span class="font-medium"><%= totalInfirmiers %></span> résultats
                            </div>
                            <div class="flex items-center gap-2">
...
<script>
    function copyToClipboard(text) {
        ...
        navigator.clipboard.writeText(text).then(function() {
            ...
            button.innerHTML = '<svg class="w-4 h-4 text-green-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>';
