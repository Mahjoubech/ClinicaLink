        ...
        <header class="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg border border-white/50 p-6 mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">Gestion des Généralistes</h1>
                        <p class="text-sm text-gray-600 mt-1">Liste et gestion des médecins généralistes</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <aside class="lg:col-span-1">
                <nav class="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg border border-white/50 p-6 sticky top-6">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <p class="text-xs font-medium text-gray-500 mb-3">Statistiques Généralistes</p>
                        <div class="space-y-2">
                            <%
                                List<Medcen> allGeneralistes = (List<Medcen>) request.getAttribute("generalistes");
                                int totalGeneralistes = allGeneralistes != null ? allGeneralistes.size() : 0;
                            %>
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-gray-600">Total généralistes</span>
                                <span class="font-semibold text-purple-600"><%= totalGeneralistes %></span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-gray-600">Dernier ajout</span>
                                <span class="font-semibold text-blue-600">
                                    <%
                                        if (totalGeneralistes > 0 && allGeneralistes.get(0).getId() != null) {
                                            out.print(allGeneralistes.get(0).getId());
                                        } else {
                                            out.print("--");
                                        }
                                    %>
            ...
            <main class="lg:col-span-3">
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">Liste des Généralistes</h2>
                        <p class="text-sm text-gray-600 mt-1">Gérez les médecins généralistes de votre établissement</p>
                    </div>
                    <a href="<%= request.getContextPath() %>/admin/create"
                ...
                <div class="backdrop-blur-sm bg-white/80 rounded-2xl shadow-lg border border-white/50 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-blue-50">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-bold text-gray-900">Médecins Généralistes</h3>
                                <p class="text-sm text-gray-600 mt-1"><%= totalGeneralistes %> généraliste(s) trouvé(s)</p>
                            </div>
                            <div class="flex items-center gap-3">
                                <div class="relative">
                                    <input type="text" placeholder="Rechercher un généraliste..."
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Généraliste</th>
                            ...
                            <%
                                List<Medcen> generalistes = (List<Medcen>) request.getAttribute("generalistes");
                                if (generalistes != null && !generalistes.isEmpty()) {
                                    for (Medcen generaliste : generalistes) {
                            %>
                            <tr class="hover:bg-purple-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-600">
                                    <%= generaliste.getId() != null ? generaliste.getId() : "N/A" %>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-green-400 flex items-center justify-center text-white text-sm font-semibold shadow-md">
                                            <%= (generaliste.getNomComplet() != null && !generaliste.getNomComplet().isEmpty()) ? generaliste.getNomComplet().substring(0,1).toUpperCase() : "G" %>
                                        </div>
                                        <div class="text-left">
                                            <p class="text-sm font-medium text-gray-900"><%= generaliste.getNomComplet() %></p>
                                            <p class="text-xs text-gray-500">ID: <%= generaliste.getId() != null ? generaliste.getId() : "N/A" %></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900"><%= generaliste.getEmail() %></div>
                                    <div class="text-sm text-gray-500"><%= generaliste.getTele() != null ? generaliste.getTele() : "Non renseigné" %></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                        <%= generaliste.getRole() != null ? generaliste.getRole().name() : "GENERALISTE" %>
                                    </span>
                                </td>
                            </tr>
                            <%
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-4 text-lg font-medium text-gray-900">Aucun généraliste trouvé</p>
                                    <p class="mt-2 text-sm text-gray-500">Commencez par ajouter votre premier médecin généraliste.</p>
                                    <a href="<%= request.getContextPath() %>/admin/create"
                                       class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-blue-500 text-white font-medium rounded-lg hover:shadow-lg transition-all">
                                        ...
                                        Ajouter le premier généraliste
                                    </a>
                                </td>
                            </tr>
                            <%
                    ...
                    <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                        <div class="flex items-center justify-between">
                            <div class="text-sm text-gray-700">
                                Affichage de <span class="font-medium">1</span> à <span class="font-medium"><%= totalGeneralistes %></span> sur <span class="font-medium"><%= totalGeneralistes %></span> résultats
                            </div>
